(() => {
  ...
  const fallbackImage = "assets/image-fallback.svg";
    ...
    return `
      <div class="mini-cart-item" data-mini-cart-name="${fullName}">
        <a href="${href}" class="mini-cart-thumb">
          <img src="${image}" alt="${safeBaseName}" loading="lazy" decoding="async">
        </a>
        ...
        <button class="mini-cart-remove" type="button" data-mini-cart-remove aria-label="Hapus ${safeBaseName} dari keranjang">
          <svg aria-hidden="true" viewBox="0 0 24 24">
            ...
            <path d="M19 6l-1 14H6L5 6"></path><path d="M10 11v5"></path>
  ...
  const render = () => {
    ...
    document.querySelectorAll(".mini-cart-dropdown").forEach((dropdown) => {
      ...
      const checkoutLink = dropdown.querySelector(
  ...
  const saveItems = (items) => {
    if (window.NexCart?.save) window.NexCart.save(items);
  ...
  document.addEventListener("click", (event) => {
    ...
    if (itemElement) removeItem(itemElement.dataset.miniCartName);
  });
  ...
  document.addEventListener("nexgear:cart-change", render);
